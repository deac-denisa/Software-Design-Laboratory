...
public class EmployeeService {
    ...
    public ResponseEntity<String> addEmployee(Employee employee) {
        if (employeeRepository.findEmployeeByEmail(employee.getEmail()) != null) {
            return new ResponseEntity<>(ResponseMessage.EMPLOYEE_ALREADY_EXISTS.getMessage(), HttpStatus.BAD_REQUEST);
        }

        employeeRepository.save(employee);

        return new ResponseEntity<>(ResponseMessage.EMPLOYEE_CREATED_SUCCESSFULLY.getMessage(), HttpStatus.CREATED);
    }
